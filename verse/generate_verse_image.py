...
Использует шаблон template_palm_leaf.png (пальмовый лист с рваными,
обожжёнными краями) и накладывает на него:
  - крупный заголовок "UTTARATANTRA" (вверху, у изображения Будды)
  - номер изречения (EN + RU)
  - текст изречения на английском (первым, крупно, жирным)
  - текст изречения на русском (вторым, крупно, жирным курсивом)
  - подпись страницы-источника
  - тонкий водяной знак "Uttaratantra" внизу слева на каждой карточке

Чёрный фон вокруг листа делается прозрачным (мягкое затухание к краям).
...
def _draw_tracked(draw, xy, text, font, fill, tracking=6, anchor_center=False):
    """Рисует текст с ручной разрядкой букв (letter-spacing)."""
...
def generate_verse_image(verse_number: int) -> Image.Image:
    ...
    # чёрный фон вокруг листа -> прозрачность (порог по яркости)
    ...
    _draw_tracked(draw, (145, 1440), "Uttaratantra", f_watermark, INK_FAINT, tracking=3, anchor_center=False)
