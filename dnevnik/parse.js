...
const MOMENT_INSTRUCTIONS = `Это личный дневник Elena — ВСЁ пиши от ПЕРВОГО ЛИЦА ("я сделала", "я почувствовала", "я радуюсь"), никогда не от третьего ("Елена сделала", "она почувствовала"). Это касается всех полей ниже, особенно text и sozhalenie.

Каждый элемент pluses — один добрый момент:
   - text: краткая (1-2 предложения) выжимка того, что произошло, ЕЁ словами, от первого лица (не выдумывай деталей, которых она не говорила)
