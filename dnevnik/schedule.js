// Шесть времён напоминаний по Бали — подтверждено Elena (сдвинуто с 7:00
// на 8:00 после теста первого дня). Если расписание понадобится поменять
// ещё раз, достаточно поправить только этот массив: остальной код
// обращается только к SLOTS, hardcoded времён больше нигде нет.
const SLOTS = [
...
// Elena: в этом же чате бота падают и другие уведомления (волонтёрство,
// передачи), поэтому окно почти до следующего слота (2.5ч = 150 минут,
// минус минута про запас) — если ответила даже через час-полтора, всё
// равно должно засчитаться на тот принцип, который был.
const PENDING_WINDOW_MINUTES = 149;
