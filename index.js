...
// ВРЕМЕННО: тест кнопок в ТЕСТОВОЙ ГРУППЕ, версия 2 (та же вымышленная
// неделя, что и в прошлый раз) — добавлена двуязычная шапка первым
// сообщением, нумерация карточек, жирное название и жирные даты внутри
// строки времени. Структуру (отдельная карточка на эфир, кнопки под
// открытыми) Elena одобрила и просила не трогать. Маркер пишется ДО
// отправки — рестарт не повторит. Удалить после проверки.
(async () => {
  const TEST_GROUP_CHAT_ID = -5172293748;
  const fs = require('fs');
  const markerPath = process.env.SIMPLE_BUTTONS_TEST2_MARKER_PATH || '/data/simple_buttons_test2_sent.json';
  if (fs.existsSync(markerPath)) return;
  try {
    fs.writeFileSync(markerPath, JSON.stringify({ at: new Date().toISOString() }));
  } catch (err) {
    console.error('[simple-buttons-test2] не удалось записать маркер, тест не отправлен:', err.message);
    return;
  }
  ...
  try {
    ...
    for (const e of events) {
      ...
      const lines = [
        `${i}. <b>${e.title}</b>`,
        `🗓 AZ: <b>${e.az}</b>, ${e.azTime} · MCK: <b>${e.msk}</b>, ${e.mskTime}`,
      ];
    ...
    console.log(`[simple-buttons-test2] отправлено в тестовую группу ${TEST_GROUP_CHAT_ID}: сообщений=${events.length + 1}`);
  } catch (err) {
    console.error('[simple-buttons-test2] ошибка отправки:', err.message);
    try {
