...
// Breakdown counts at the top still cover every event, same as always.
...
function buildWeeklyMessage(events, range, { failedTabs = [], now = new Date(), tags = [] } = {}) {
  ...
  if (events.length === 0) {
    ...
    if (missing.length > 0) {
      enSectionParts.push('', missing.map(weeklyEventBlockEn).join('\n\n'));
      ruSectionParts.push('', missing.map(weeklyEventBlockRu).join('\n\n'));
